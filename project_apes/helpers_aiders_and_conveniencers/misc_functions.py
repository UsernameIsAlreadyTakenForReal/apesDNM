...
def get_last_model(Logger, solution_name, app_instance_metadata):
    ...
    if len(list_of_finds) != 0:
        incomplete_full_path = os.path.abspath(list_of_finds[0])
        path_elements = incomplete_full_path.split(os.sep)
        folder_structure_from_project_root_to_model_dir = (
            app_instance_metadata.shared_definitions.project_model_root_path[2:]
        )
        new_path = ""
